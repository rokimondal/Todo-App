'use client'
import { FaSearch, FaBars, FaPlus, FaTasks, FaStickyNote, FaUser } from "react-icons/fa"
...
export default function Sidebar() {
    const { setOpenAddTask, setOpenSearch, openSidebar, setOpenSidebar } = useContext(TaskContext);

    return <div>
        <div className={`${!openSidebar && "hidden"} fixed z-50 w-full h-full`}>
            <div className="lg:hidden fixed w-full h-full inset-0 z-50 bg-black bg-opacity-75" onClick={() => setOpenSidebar(false)}>

                {openSidebar && <div className="max-w-64 flex min-h-[calc(100vh-2rem)] lg:max-w-64 lg:min-w-64 bg-gray-200 m-4 p-4 rounded-lg  flex-col justify-between">
                    <div >
                        <div className="flex justify-between items-center mb-2 text-gray-600">
                            <span className=" font-Montserrat font-semibold text-2xl">Menu</span>
                            <button className="text-gray-600 text-lg" onClick={() => setOpenSidebar(false)}><FaBars /></button>
                        </div>
                        <div className="mb-2">
                    ...
                    <div>
                        <div className="mt-4 mr-10">
                            <Link href={'/profile'} className="text-[1rem] font-semibold text-gray-600 mb-1 flex justify-center items-center hover:text-gray-500 hover:cursor-pointer"><FaUser className="mr-2 text-xl" />Profile</Link>
        ...
        <div className=" hidden lg:flex min-h-[calc(100vh-2rem)] lg:max-w-64 lg:min-w-64 bg-gray-200 m-4 p-4 rounded-lg  flex-col justify-between">
            <div >
                <div className="flex justify-between items-center mb-2 text-gray-600">
                    <span className=" font-Montserrat font-semibold text-2xl">Menu</span>
