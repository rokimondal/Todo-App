import Page from '@/components/addSticky';
...
const page = () => {
  ...
  return (
    <div className='mt-4 p-2 rounded-lg w-full grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5  m-auto'>
      <Page />
      {pendingTask.length > 0 && pendingTask.map(task => (<div key={task._id} className=' relative m-4 p-4 rounded-md h-48 overflow-scroll flex-col  justify-between opacity-90' style={{ backgroundColor: `#${colors[Math.floor(Math.random() * 100)]}` }}>
        <h2 className='text-lg min-h-14'>{task.title}</h2>
        <h2 className='text-sm mt-2 h-16'>{task.description}</h2>
        <div className="absolute top-0 left-0 w-full h-full bg-zinc-700 flex items-center space-x-10 justify-center transition-all duration-300 opacity-0 hover:opacity-[0.98]">
          <button className="text-gray-500 text-2xl font-semibold hover:text-gray-900 transition-colors duration-300">
            Edit
          </button>
          <button className="text-red-600 text-2xl font-semibold hover:text-red-900 transition-colors duration-300">
            Delete
          </button>
        </div>
      </div>))}
